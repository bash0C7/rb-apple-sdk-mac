...
class TestDocDialog < Test::Unit::TestCase
  ...
  # llm_resolver fallback (Step 7.2). Used when the primary KB-driven
  # resolver returns nil — Swift-overlay Apple symbols and Ruby
  # stdlib expressions both rely on this path to surface anything.
  def test_render_falls_through_to_llm_resolver_when_primary_nil
    primary = make_resolver({})  # always nil
    fallback = make_resolver(
      "Apple::Foundation::URL.appendingPathComponent" => "Generated by LLM."
    )
    dialog = DocDialog.new(resolver: primary, llm_resolver: fallback)
    out = dialog.render(
      matched: "Apple::Foundation::URL.appendingPathComponent",
      cursor_pos: cursor(0, 0), max_height: 10
    )
    assert out.is_a?(Reline::DialogRenderInfo)
    assert_includes out.contents, "Generated by LLM."
  end

  def test_render_skips_llm_resolver_when_primary_resolves
    primary = make_resolver(
      "Apple::CoreFoundation::CFArrayAppendValue" => "KB authoritative doc."
    )
    fallback_called = false
    fallback = Object.new
    fallback.define_singleton_method(:resolve) do |_m|
    ...
    refute fallback_called, "llm_resolver must not run when primary returned non-nil"
    assert_includes out.contents, "KB authoritative doc."
  end

  def test_render_returns_nil_when_both_resolvers_miss
    dialog = DocDialog.new(
      resolver: make_resolver({}),
      llm_resolver: make_resolver({})
    )
    out = dialog.render(
      matched: "String.to_s", cursor_pos: cursor(0, 0), max_height: 10
    )
    assert_nil out
  end
end
