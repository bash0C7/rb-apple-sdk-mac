...
class TestPipelineParallelBitIdentical < Test::Unit::TestCase
  ...
  def test_n1_n2_n4_n8_with_K4_bit_identical_symbols
    omit "set APPLE_SDK_MAC_KB_INTEGRATION=1 to run" unless ENV["APPLE_SDK_MAC_KB_INTEGRATION"]
    n1 = rebuild_with(workers: 1, framework_parallelism: 1)
    n2 = rebuild_with(workers: 2, framework_parallelism: 2)
    n4 = rebuild_with(workers: 4, framework_parallelism: 2)
    n8 = rebuild_with(workers: 8, framework_parallelism: 4)
    assert_equal n1, n2, "N=1 と N=2 K=2"
    assert_equal n1, n4, "N=1 と N=4 K=2"
    assert_equal n1, n8, "N=1 と N=8 K=4"
  end
end
